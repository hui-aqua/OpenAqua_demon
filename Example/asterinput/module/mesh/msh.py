...
for i in range(0, NT):
    for j in range(NN):
        point_one_cage.append(
            [cr_top * np.cos(i * 2 * pi / float(NT)),
             cr_top * np.sin(i * 2 * pi / float(NT)),
              - j * cage_height / float(NN)])
    for j in range(BN):
        point_one_cage.append(
            [cr_top * ((BN - j) / BN) * np.cos(i * 2 * pi / float(NT)),
             cr_top * ((BN - j) / BN) * np.sin(i * 2 * pi / float(NT)),
...
for j in range(NT_frame):
    # top
    con_in_one_frame_top.append([1+j*2*(BN+NN),1+j*(NN_frame+1)+number_of_point_one_netting])
    # circular
    if j<NT_frame-1:
    ...
    # bottom                  --> netting , lines 
    for i in range(int(NN_frame/2)):
        con_in_one_frame_line.append([1+NN+2*j*(BN+NN)-2*i,1+NN_frame+j*(NN_frame+1)+number_of_point_one_netting-1-2*i])   
         
    # con_in_one_frame_line.append([1+NN+2*j*(BN+NN)-0,1+NN_frame+j*(NN_frame+1)+number_of_point_one_netting-2])
    # con_in_one_frame_line.append([1+NN+2*j*(BN+NN)-2,1+NN_frame+j*(NN_frame+1)+number_of_point_one_netting-4])
    # con_in_one_frame_line.append([1+NN+2*j*(BN+NN)-4,1+NN_frame+j*(NN_frame+1)+number_of_point_one_netting-6])
...
total_volume_of_twine=float(np.pi*0.25*pow(dws,2)*total_length_of_twine)

### write to the meshinfo:
fb_netting=total_volume_of_twine*9.81*parameters['Environment']['fluidDensity']/len(point)
section_twine=np.pi*0.25*pow(dws,2)
...
meshinfo = {
    ...
    'dwh':dwh,
    'caseInfo':parameters,
    'weight_NT':parameters['Frame']['weight_per_metter']*cr_top/NT_frame,
}

try:
    Mesh_1.ExportMED(os.path.join(cwd,meshname))
    pass
except:
...
with open(os.path.join(cwd,'meshInformation.py'), 'w') as f:
    f.write('meshinfo=')
