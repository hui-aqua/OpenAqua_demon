    ...
    def hydro_coefficients(self, current_velocity):
        ...
        if self.model_index not in 'M1,M2,M3,M4,M5':
            ...
            if 0 < reynolds_number < 1:
                ...
                print("Now, norm of current_velocity is "+ str(np.linalg.norm(current_velocity)))
    ...
    def force_on_element(self, node_position, velocity_fluid, velocity_structure=np.zeros((99999, 3)),elevation=False):
        ...
        for index in range(num_line):
            ...
            if elevation is not False:
                ...
                if element_center[2] > element_elevation[2]:   
                    row=row_air
                else:
                    row=row_water 
            ft = 0.5 * row * self.dwh * (element_length - self.dwh) * drag_t * np.dot(element_direction,relative_velocity) * element_direction * np.linalg.norm(np.dot(element_direction, relative_velocity))
            fn = 0.5 * row * self.dwh * (element_length - self.dwh) * drag_n * (relative_velocity - np.dot(element_direction,relative_velocity) * element_direction) * np.linalg.norm((relative_velocity - np.dot(element_direction, relative_velocity) * element_direction))
