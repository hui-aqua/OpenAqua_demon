...
class screenModel:
    ...
    def __init__(self, model_index, hydro_element, solidity, dw0=0.0, rho=1140.0):
        """
    ...
    def cal_buoy_force(self, node_position, elevation):
        ...
        for index, panel in enumerate(self.triangular_elements):  # loop based on the hydrodynamic elements
            ...
            element_volume = element_area * self.sn * self.dw0 * 0.25 * pi
            element_center = (p1 + p2 + p3) / 3.0
            element_elevation = (elevation[int(panel[0])] + elevation[int(panel[1])] + elevation[int(panel[2])]) / 3.0
            if element_center[2] > element_elevation[2]:
                force_on_element[index] = [0, 0, element_volume * gravity * (row_air-self.rho)]
            else:
                force_on_element[index] = [0, 0, element_volume * gravity * (row_water-self.rho)]
